...
for (i in (1:length(ChIPAlignFileList))) {
	...
	if (tools::file_ext(ChIPAlignFileList[i]) == "gz") {
		...
		if (i == 1){
			system(paste("bedmap --echo --mean --prec 3", TargetBinnedChrFile , ChIPAlignFileList[i], "| awk","'{sub(/\\|/,\"\t\"); print $0}'",">", MergedChIPCovFile )) #seperate column by "|"
		} else {
			system(paste("bedmap --mean --prec 3", TargetBinnedChrFile,ChIPAlignFileList[i], ">", tempfile1 ))
